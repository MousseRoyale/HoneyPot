...
#feel free to get your own key at https://ipgeolocation.io/

$API_KEY      = "ac370a9fabfc4d96a8b65edb71c21dc7"
$LOGFILE_NAME = "failed_rdp.log"
$LOGFILE_PATH = "C:\ProgramData\$($LOGFILE_NAME)"

# This filter event 4625 (failed logon) from Windows Event Viewer
$XMLFilter = @'
<QueryList> 
...
while ($true)
{
    ...
    # Retrieves events from Windows Event Viewer based on the filter every second
    ...
    foreach ($event in $events) {
        ...
        if ($event.properties[19].Value.Length -ge 5) {
            ...
            if (-Not ($log_contents -match "$($timestamp)") -or ($log_contents.Length -eq 0)) {
                ...
                #if you want to add ISP provider add: $isp = $responseData.isp

                # Write all gathered data to the custom log file. It will look something like this:
                #
                "latitude:$($latitude),longitude:$($longitude),destinationhost:$($destinationHost),username:$($username),sourcehost:$($sourceIp),state:$($state_prov), country:$($country),label:$($country), $($state_prov) - $($sourceIp),timestamp:$($timestamp)" | Out-File $LOGFILE_PATH -Append -Encoding utf8

                Write-Host -BackgroundColor Black -ForegroundColor Magenta "latitude:$($latitude),longitude:$($longitude),destinationhost:$($destinationHost),username:$($username),sourcehost:$($sourceIp),state:$($state_prov), label:$($country), $($state_prov) - $($sourceIp),timestamp:$($timestamp)"
            }
            else {
